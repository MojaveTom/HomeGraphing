...
# for when logging to file doesn't work, these will direct
# all output to stdout.
# debug = warning = info = error = critical = print
class Palette(object):
    ...
    def __init__(self, colorName, num=None, *args, **kwargs):
        ...
        debug('Creating palette for colorName: %s with %s colors.' % (colorName, num))
        ...
        try:
            ...
            if colorName not in paletteFamilies:
                warning('"%s" is neither a known color name nor a known color palette name; using Dark2[8].' % colorName)
                return
            pass
        _num = num
        ...
        if rptcnt == 1:
            debug('Generating pallette of %s colors from %s of %s.' % (_num, minContainPalette, pf[minContainPalette]))
    ...
    def nextColor(self, group, index=None):
        ...
        if len(self._Palette) == 0:
            debug('Attempting to get a color from an empty palette.')
            return None
        ind = ind % len(self._Palette)
        debug('Requesting color at index %s.', ind)
        if group not in self.callGroups.keys():
            debug('Creating new "nextColor" call group named "%s"', group)
            self.callGroups[group] = ind
        if index is not None:
            self.callGroups[group] = ind
        ind = self.callGroups[group]
        debug('Retrieving color at index %s (goup is "%s").' % (ind, group))
        self.callGroups[group] = (ind + 1) % len(self._Palette)
        debug('Next color index for group "%s" is %s; color is: %s.' % (group, ind, self._Palette[ind]))
